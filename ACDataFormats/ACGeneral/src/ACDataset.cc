...
double ACDataset::xsection() const {
    if (isData_) printf("ACDataset::luminosity: Warning! Your are accessing a value not stored for data.\n");
    return xsection_;
}
float ACDataset::luminosity() const {
    if (!isData_) printf("ACDataset::luminosity: Warning! Your are accessing a value not stored for MC.\n");
    return luminosity_;
}
double ACDataset::norm(double desiredLumi) const {
    ...
    if (isData_) {
        ...
        if (initialEvents() > 0.0) ratio = scale()*xsection()*desiredLumi/initialEvents();
    }
